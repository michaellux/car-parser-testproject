...
class CarScraper
{
    ...
    public function getCarLinks(): array
    {
        $allLinks = [];
        ...
        for ($currentPage = $this->startPage; $currentPage <= $finishPage; $currentPage++) {
            ...
            $links = $crawler->filter('article.listing-item .listing-title > a')->each(function (Crawler $node) {
    ...
    private function getUrl($page = 1): string
    {
        return 'https://premiumcarsfl.com/listing-list-full' . '/page/' . $page;
    }

    private function getFinishPage(): int
    {
    ...
    private function getExpectedCarQuantity(): string
    {
        $response = $this->client->request('GET', $this->getUrl());
        $html = $response->getBody();
        $crawler = new Crawler($html);
        preg_match('/(\d+)\D*$/', $crawler->filter('.results-count')->text(), $m);
        return $m[1];
    }

    public function checkResultCount($allCarLinks): bool
    {
